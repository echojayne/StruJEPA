...
def torch_mha_forward_from_weights(
    x: torch.Tensor,
    *,
    ...
    q_weight, k_weight, v_weight = in_proj_weight.split(embed_dim, dim=0)
    if in_proj_bias is not None:
        q_bias, k_bias, v_bias = in_proj_bias.split(embed_dim, dim=0)
    else:
        q_bias = k_bias = v_bias = None
    ...
    q = q.view(batch_size, seq_len, int(num_heads), head_dim).transpose(1, 2)
    k = k.view(batch_size, seq_len, int(num_heads), head_dim).transpose(1, 2)
    v = v.view(batch_size, seq_len, int(num_heads), head_dim).transpose(1, 2)

    additive_mask = build_additive_attention_mask(
        attn_mask,
        key_padding_mask,
        batch_size=batch_size,
        active_heads=int(num_heads),
        target_len=seq_len,
        source_len=seq_len,
        device=x.device,
        dtype=x.dtype,
    ...
    output = F.linear(context, out_proj_weight, out_proj_bias)
    if not batch_first:
        output = output.transpose(0, 1).contiguous()
    return output
...
def qkv_attention_forward_from_weights(
    x: torch.Tensor,
    *,
    q_weight: torch.Tensor,
    q_bias: torch.Tensor | None,
    ...
    batch_size, seq_len, _ = x.shape
    active_dim = int(num_heads) * int(head_dim)
    q = F.linear(x, q_weight, q_bias)
    k = F.linear(x, k_weight, k_bias)
    v = F.linear(x, v_weight, v_bias)

    q = q.view(batch_size, seq_len, int(num_heads), int(head_dim)).transpose(1, 2)
    k = k.view(batch_size, seq_len, int(num_heads), int(head_dim)).transpose(1, 2)
    v = v.view(batch_size, seq_len, int(num_heads), int(head_dim)).transpose(1, 2)
    if q_norm is not None:
        q = apply_layer_norm(q_norm, q)
    if k_norm is not None:
        k = apply_layer_norm(k_norm, k)
    ...
    if head_mask is not None:
        if head_mask.ndim == 1:
            context = context * head_mask[: int(num_heads)].view(1, int(num_heads), 1, 1)
        elif head_mask.ndim == 2:
            context = context * head_mask[:, : int(num_heads)].view(batch_size, int(num_heads), 1, 1)
    context = context.transpose(1, 2).contiguous().view(batch_size, seq_len, active_dim)
    if out_norm is not None:
        context = apply_layer_norm(out_norm, context)
    output = F.linear(context, out_weight, out_bias)
    if out_dropout is not None:
        output = out_dropout(output)
    return output
...
def ffn_forward_from_weights(
    ...
    *,
    ...
    if norm is not None:
        hidden = apply_layer_norm(norm, hidden)
    output = F.linear(hidden, fc2_weight, fc2_bias)
    if dropout2 is not None:
        output = dropout2(output)
    return output
