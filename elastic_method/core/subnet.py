...
def select_depth_indices(*, total_layers: int, active_layers: int) -> list[int]:
    ...
    strategy = os.environ.get("STRUJEPA_DEPTH_SELECTION", "").strip().lower()
    if strategy in {"prefix", "sequential", "first", "ofa_stage_prefix"}:
        return list(range(active_layers))
    if strategy in {"dynabert", "dynabert_every_other", "every_other"}:
        return [
            max(0, math.floor((layer_idx + 1) * total_layers / active_layers) - 1)
            for layer_idx in range(active_layers)
        ]
    if strategy in {"dynabert_original", "dynabert_prefix_spaced"}:
        return [
            max(0, math.floor(layer_idx * total_layers / active_layers))
            for layer_idx in range(active_layers)
        ]
    if strategy not in {"", "uniform", "uniform_tail", "default"}:
        raise ValueError(
            "STRUJEPA_DEPTH_SELECTION must be one of: prefix, sequential, first, "
            "dynabert, dynabert_every_other, every_other, dynabert_original, "
            "dynabert_prefix_spaced, ofa_stage_prefix, uniform, uniform_tail, default"
        )
    return [
        max(0, math.floor((layer_idx + 1) * total_layers / active_layers) - 1)
        for layer_idx in range(active_layers)
