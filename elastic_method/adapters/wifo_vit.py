...
from elastic_method.core.ops import (
    elastic_ffn_forward,
    elastic_qkv_attention_forward,
    ffn_forward_from_weights,
    qkv_attention_forward_from_weights,
)
    ...
    def forward(self, x):
        ...
        if runtime is not None and runtime.completion_module is not None:
            q_weight, q_bias, k_weight, k_bias, v_weight, v_bias, out_weight, out_bias = (
                runtime.completion_module.complete_wifo_attention(self, active_heads=active_heads)
            )
            attn_output = qkv_attention_forward_from_weights(
                self.norm1(x),
                q_weight=q_weight,
                q_bias=q_bias,
                k_weight=k_weight,
        ...
        attn_residual = self.drop_path(attn_output)
        x = x + attn_residual
        if runtime is not None and runtime.completion_module is not None:
            fc1_weight, fc1_bias, fc2_weight, fc2_bias = runtime.completion_module.complete_wifo_ffn(
                self,
                active_ffn_dim=active_ffn_dim,
            )
            ffn_output = ffn_forward_from_weights(
                self.norm2(x),
        ...
        ffn_residual = self.drop_path(ffn_output)
        x = x + ffn_residual
...
@register_block_adapter
def _register_wifo_vit_width_completion_adapter() -> BlockAdapter:
    return WifoVitAdapter(family="wifo_vit_width_completion")
