#!/usr/bin/env python
"""Run StruJEPA on the benchmark static iTransformer traffic forecaster."""
...
from integrations.traffic_forecasting.strujepa import (  # noqa: E402
    ITRANSFORMER_CHECKPOINT,
    ITRANSFORMER_CONFIG,
    build_itransformer_loaders,
    build_itransformer_strujepa,
    load_yaml,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default=str(ROOT / "configs/traffic_forecasting/current_itransformer.yaml"),
    )
...
def build_spec(payload: dict[str, Any], *, family: str) -> ElasticizationSpec:
    return ElasticizationSpec(
        stack_path=str(payload.get("stack_path", "encoder")),
        block_family=family,
...
def main() -> int:
    ...
    checkpoint_path = Path(checkpoint_raw) if checkpoint_raw else ITRANSFORMER_CHECKPOINT
    source_config_raw = model_cfg.get("source_config_path")
    source_config_path = Path(source_config_raw) if source_config_raw else ITRANSFORMER_CONFIG

    model, task_adapter = build_itransformer_strujepa(
    ...
    train_loader, val_loader = build_itransformer_loaders(
        config_path=source_config_path,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if should_validate:
            ...
            metric = float(val_metrics.get("mae", val_metrics.get("loss", float("inf"))))
