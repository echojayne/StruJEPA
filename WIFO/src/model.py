...
except Exception:  # pragma: no cover - WiFo can still run without elastic_method installed
...
class WiFo(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio, mask_strategy, seed=None, data=None, mode='backward', scale=None):
        # embed patches
        N, _, T, H, W = x.shape
        generator = _make_generator(seed, x.device)
        ...
        if mask_strategy == 'random':
            x, mask, ids_restore, ids_keep = random_masking(x, mask_ratio, generator=generator)
        ...
        if runtime is not None and runtime.return_encoder_state:
            runtime.last_encoder_state = x_attn
    ...
    def forward(self, imgs, mask_ratio=0.5, mask_strategy='random',seed=None, data='none'):
        if torch.is_tensor(imgs):
            if imgs.dim() == 6:
                ...
                raise ValueError(f"Expected a 5D/6D tensor for WiFo input, got shape {tuple(imgs.shape)}")
        else:
            imgs = torch.stack(list(imgs)).squeeze(1)
        generator = _make_generator(seed, imgs.device)

        snr_db = 20
        noise_power = torch.mean(imgs ** 2) * 10 ** (-snr_db / 10)
        noise = torch.randn(imgs.shape, device=imgs.device, dtype=imgs.dtype, generator=generator) * torch.sqrt(noise_power)
        imgs_n = imgs + noise
